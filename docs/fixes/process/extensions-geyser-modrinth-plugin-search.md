...
| DONE | 4. Harden download resolution | The original fix made Modrinth tolerate broad search-row version ids; later work moved that tolerance to the quick-add caller so exact version selections stay exact. |
...
The marketplace UI can ask providers to resolve the version id stored on a search result. For Modrinth, that id may not be present after the provider narrows versions by active loader and Minecraft version. The durable behavior is to let quick-add ask for the newest compatible build without an exact version id, while preserving exact semantics for user-selected version options.
